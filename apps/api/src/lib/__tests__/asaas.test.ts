...
test('--- Integração Asaas Modo Leitura & Webhook Suite (Hardening Etapa 4B) ---', async (t) => {
  ...
  await t.test('27. Cobrança com deleted: true ou PAYMENT_DELETED (PNEUTEK R$ 1.594,00) é excluída de Pendentes/Em Aberto e não infla os KPIs', async () => {
    const now = new Date();
    const currentMonthDueDate = new Date(now.getFullYear(), now.getMonth() + 1, 0, 23, 59, 59, 999);
  ...
  await t.test('28. Sincronização atualiza cobrança existente no banco de PENDING para DELETED via deleted: true', async () => {
    const now = new Date();
    const currentMonthDueDate = new Date(now.getFullYear(), now.getMonth() + 1, 0, 23, 59, 59, 999);
    const currentMonthDueDateStr = `${currentMonthDueDate.getFullYear()}-${String(currentMonthDueDate.getMonth() + 1).padStart(2, '0')}-${String(currentMonthDueDate.getDate()).padStart(2, '0')}`;
  ...
  await t.test('31. syncAllWallet reconcilia cobrança PENDING omitida da listagem geral via GET individual /payments/{id}', async () => {
    ...
    const currentMonthDueDate = new Date(now.getFullYear(), now.getMonth() + 1, 0, 23, 59, 59, 999);
    const currentMonthDueDateStr = `${currentMonthDueDate.getFullYear()}-${String(currentMonthDueDate.getMonth() + 1).padStart(2, '0')}-${String(currentMonthDueDate.getDate()).padStart(2, '0')}`;
    const pneutekDueDateStr = `${currentMonthDueDate.getFullYear()}-${String(currentMonthDueDate.getMonth() + 1).padStart(2, '0')}-25`;
