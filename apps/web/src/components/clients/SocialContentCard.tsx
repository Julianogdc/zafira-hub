...
export function SocialContentCard({ post, clientId, account }: SocialContentCardProps) {
  ...
  // Informações de formato canônico
  ...
  const thumbnailUrl = primaryMedia?.url || null;
  const hasValidMedia = !!thumbnailUrl && !imageError;

  // Resolução da conta e plataforma
  const platformState = post.platformStates?.[0];
  const platform = platformState?.platform || account?.platform || 'INSTAGRAM';
  const accountName = account?.accountName || 'Conta social';
  const accountPicture = account?.accountPicture || null;
  const releaseUrl = platformState?.permalink || null;
  ...
  const handleClick = () => {
    if (post.status === 'PUBLISHED' && releaseUrl && releaseUrl.trim() !== '') {
  ...
  const ariaLabel =
    post.status === 'PUBLISHED' && releaseUrl
      ? `Abrir publicação no ${platform} em nova aba`
      : `Abrir prévia de ${formatText} ${statusDesc} no ${platform}`;

  // Badge de status canônico
    ...
    switch (status) {
      ...
      case 'PARTIALLY_PUBLISHED':
        return (
          <Badge
            variant="outline"
            className="bg-amber-500/10 text-amber-300 border-amber-500/20 text-[11px] gap-1 font-medium"
          >
      ...
      case 'PUBLISHING':
        return (
          <Badge
            variant="outline"
            className="bg-sky-500/10 text-sky-400 border-sky-500/20 text-[11px] gap-1 font-medium"
          >
      ...
      case 'CANCELLED':
        return (
          <Badge
            variant="outline"
            className="bg-zinc-600/10 text-zinc-400 border-zinc-600/20 text-[11px] gap-1 font-medium"
          >
  ...
  // Badge específico do formato canônico
  const renderFormatBadge = () => {
    ...
    return (
      <Badge variant="outline" className="bg-zinc-500/10 text-zinc-300 border-white/10 text-[10px] font-semibold">
        Feed
      </Badge>
    );
  };

  // Auxiliar para identificar vídeo
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 p-4 flex flex-col justify-between min-w-0 space-y-3">
        ...
        {/* Linha 4: Rodapé com data, horário e dica visual */}
          ...
          <div className="flex items-center gap-1.5 min-w-0 truncate">
            {post.status === 'DRAFT' ? (
              ...
              <>
                ...
                <span className="truncate text-zinc-400 font-medium">Agendamento cancelado</span>
              </>
            ) : (
              <>
